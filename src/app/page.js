...
function ScrollProgress() {
  ...
  return <motion.div className="fixed top-0 left-0 right-0 h-1 bg-blueprint origin-left z-[9999]" style={{ scaleX }} />;
}
      ...
      <main className="min-h-screen flex flex-col justify-center px-4 sm:px-6 md:px-24 max-w-7xl mx-auto pt-28 pb-12 md:pt-20">
        ...
        <div className="max-w-4xl relative">
          ...
          >
            ...
            {/* Blinking Semester Tag */}
            <span className="border border-blueprint px-2 py-0.5 bg-card text-blueprint whitespace-nowrap animate-system-pulse">
              STATUS: GIKI // 5TH_SEM
            </span>

            <span className="border border-steel px-2 py-0.5 bg-card flex items-center gap-1 whitespace-nowrap">
              <span className="w-1.5 h-1.5 bg-eng-green inline-block animate-pulse"></span>
              SYS_ONLINE
            </span>
          </motion.div>
          ...
          >
            <a href="#projects" className="btn-sys w-full sm:w-auto text-center px-8 py-3.5 bg-blueprint text-white text-sm font-tech font-bold hover:bg-blueprint/90 border border-transparent hover:border-amber hover:text-amber">
              EXECUTE: VIEW_WORK
            </a>
            <a href="https://github.com/CodeBySRY" target="_blank" rel="noopener noreferrer" className="btn-sys w-full sm:w-auto text-center px-8 py-3.5 bg-card text-charcoal border border-steel text-sm font-tech font-bold hover:border-blueprint hover:bg-paper">
              ACCESS: GITHUB
            </a>
          </motion.div>
        </div>
